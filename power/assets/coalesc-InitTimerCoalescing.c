  ...
  *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
  ...
  if ( result >= 0 )
  {
      ...
      && !v20[0] )                              // 0 == 0
    {
      for ( i = 0; i < 3; ++i )
      {
        if ( v20[i + 1] )                       // 1-3 must be zero
          return ZwClose(KeyHandle);
      }
      for ( j = 0; j < 4; ++j )
      {
        if ( v20[j + 8] )                       // 8-11 must be zero
          return ZwClose(KeyHandle);
      }
      for ( k = 0; k < 4; ++k )
      {
        if ( v20[k + 16] )                      // 16-19 must be zero
          return ZwClose(KeyHandle);
      }
      for ( m = 0LL; (unsigned int)m < 4; m = (unsigned int)(m + 1) )
      {
        if ( v20[(unsigned int)m + 4] > 0x7FFFFFF5u )// 4-7 <= 0x7FFFFFF5
          return ZwClose(KeyHandle);
      }
      while ( v0 < 4 )
      {
        if ( v20[v0 + 12] > 0x7FFFFFF5u )       // 12-15 <= 0x7FFFFFF5
          return ZwClose(KeyHandle);
        ++v0;
      }
